...
const Sems = () => {
    ...
    const addsem = async (e) => {
        e.preventDefault();
        console.log("dept and name =",dept,"hii",name);

        const { data } = await axios.post(`http://localhost:3000/api/v2/sem`, {
            department: dept,
            name: name
        });
        if (data.success) {
    ...
    const getdepid = async () => {
        ...
        console.log(data)
        console.log("department = ",data.user.department._id)
    ...
    const getsems = async () => {
        setloader(true)
        console.log(dept)
        const response = await axios.get(`http://localhost:3000/api/v2/sems/${dept}`)
    ...
    const delsem = async (id) => {
        ...
        if (confirmed === "del") {
            console.log(id);
            const { data } = await axios.delete(`http://localhost:3000/api/v2/semdel/${id}`);
            if (data.success) {
