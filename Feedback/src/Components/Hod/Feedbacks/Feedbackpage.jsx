...
const Feedbackpage = () => {
    ...
    const feedbacks = async () => {
        ...
        try {
            setloader(true)
            const { data } = await axios.get(`http://localhost:3000/api/v2/feedback/${id.id}`)
    ...
    const getbyyear = async (year) => {
        ...
        const { data } = await axios.post("http://localhost:3000/api/v2/fbacksemyr", {
            sem: id.id,
            year: year
        })
        console.log(data)
    ...
    const getyshift = async (s) => {
        ...
        console.log(s)
        const { data } = await axios.post("http://localhost:3000/api/v2/feedbackshift", {
            shift: s, sem: id.id
    ...
    const getbysub = async (subject) => {
        ...
        const { data } = await axios.post("http://localhost:3000/api/v2/feedbacksub", {
            sub: subject
        })
        console.log(data)
        setfback(data.feedback)
    ...
    const subjects = async () => {
        try {
            console.log(id)
            const response = await axios.post("http://localhost:3000/api/v3/subjectsq", {
                sem: id.id
            })
    ...
    const handleSubmit = async () => {
        try {
            const response = await axios.post(`http://localhost:3000/api/v2/semesters/${enabled ? 'disable' : 'enable'}`, {
                id: id.id
            });
    ...
    const shifts = async () => {
        const { data } = await axios.get("http://localhost:3000/api/v1/shifts")
        console.log(data)
