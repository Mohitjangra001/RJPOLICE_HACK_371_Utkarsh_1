...
const Hhome = () => {
    ...
    const getdata = async () => {
        const { data } = await axios.post("http://localhost:3000/api/v2/getdata", {
            dep: dep
        });
    ...
    const getsems = async () => {
        console.log(dep)
        const response = await axios.get(`http://localhost:3000/api/v2/sems/${dep}`)
        console.log("ssss", response.data.sems)
        setsems(response.data.sems)
    }

    const fdata = async () => {
        const { data } = await axios.get("http://localhost:3000/api/v2/countf")
        setFeedbackData(data)
        console.log(data)

        const response = await axios.get(`http://localhost:3000/api/v2/countsA/64dbc5db25871abf6b82b2e4`)
