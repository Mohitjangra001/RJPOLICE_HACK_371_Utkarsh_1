...
const Ecform = () => {
    ...
    const quesions = async () => {
        try {
            setloader(true);
            const { data } = await axios.get("http://localhost:3000/api/v3/ecques");
            console.log(data);
    ...
    const addq = async (e) => {
        e.preventDefault();
        const { data } = await axios.post("http://localhost:3000/api/v2/ecquestion", {
            question: text
        });
        if (data.success) {
    ...
    const deleq = async (id) => {
        ...
        if (confirmed) {
            console.log(id);
            try {
                const { data } = await axios.delete(`http://localhost:3000/api/v2/ecqdel/${id}`);
